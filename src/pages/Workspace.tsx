...
const maybeQuoteIdentifier = (name: string): string => {
  const needsQuotes = 
    ...
    [
      "select", "from", "where", "table", "order", "group", "by", 
      "limit", "offset", "insert", "update", "delete", "create", "alter", 
      "drop", "grant", "revoke", "all", "distinct", "as", "join", "on", 
      "inner", "outer", "left", "right", "full", "union", "except", "intersect","user"
...
export function Workspace() {
  ...
  const persistSession = useCallback((currentTabs: WorkspaceTab[], activeId: string, currentActiveConnId: string | null, globalSavedConnId: string | null) => {
  ...
  const handleUpdateCell = async (column: string, newValue: string | null, originalRow: any[], columns: string[]) => {
    const targetConnectionId = activeTab.connectionId;
    if (!targetConnectionId || !activeTab.selectedTable) {
    ...
    try {
      ...
      const rowIdentifiers = originalRow.map((val, idx) => {
        ...
        const colDef = activeTab.columnDefs.find((c) => c.name === colName);
      ...
      let pkColNames = new Set(activeTab.columnDefs.filter((c) => c.is_pk).map((c) => c.name));
      if (pkColNames.size === 0) {
        pkColNames = new Set(activeTab.columnDefs.filter((c) => c.is_unique).map((c) => c.name));
      }
      if (pkColNames.size > 0) {
      ...
      // We stored it as "schema.table" or fully qualified?
      // handleSelectTable sets it to `${schema}.${table}`.
      // Note: if schema or table has dots this fails. Simplistic split.
      // usage of activeTab.selectedTable
      const [schema, table] = activeTab.selectedTable!.split(".");

      const targetColDef = activeTab.columnDefs.find((c) => c.name === column);
  ...
  const handleDeleteRow = (row: any[]) => {
    if (!activeTab.selectedTable || !activeTab.results) return;

    const [schema, table] = activeTab.selectedTable.split(".");

    // Identify PKs or Unique Keys
    let pkCols = activeTab.columnDefs.filter((c) => c.is_pk);
    if (pkCols.length === 0) {
      pkCols = activeTab.columnDefs.filter((c) => c.is_unique);
    }
    ...
    const colNames = activeTab.results.columns;
